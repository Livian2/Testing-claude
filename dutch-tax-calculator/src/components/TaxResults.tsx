...
export default function TaxResults({ result }: Props) {
  const {
    ...
    portfolioGainLoss, actualSavingsInterest,
  } = result;

  const grossIncome = box1.taxableIncome; // after deductions
            ...
            <div>
              ...
              <div className="bg-slate-50 rounded-xl p-3 border border-slate-100">
                <div className="flex text-xs text-slate-400 font-medium mb-2 gap-3">
                  ...
                  <span className="w-24 text-right">Belasting</span>
                </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div className="p-6 space-y-4">
          <div className="flex items-start gap-2 bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-800">
            ...
            <span>
              Berekening o.b.v. <strong>overgangswetgeving 2025</strong>. Fictief rendement: spaargeld <strong>1,44%</strong>,
              overige bezittingen <strong>5,88%</strong>, schulden <strong>2,62%</strong>. Tarief: <strong>36%</strong>.
              Heffingvrij: <strong>€57.000</strong> / <strong>€114.000</strong> (partners). Schuldendrempel: <strong>€3.400</strong>.
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            ...
            <StatCard label="Heffingvrij"           value={fmt(box3.exemption)}     color="green" />
        ...
        <div className="p-6 space-y-1">
          {[
            ...
            <div key={i} className="flex justify-between items-center py-2 border-b border-slate-100 last:border-0 text-sm">
              <span className="text-slate-600">{row.label}</span>
              <span className={`font-medium ${row.color}`}>{row.sign} {fmt(row.value)}</span>
            </div>
          ))}
