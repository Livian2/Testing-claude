...
const OPTIONAL_FIELDS: ToggleField[] = [
  ...
  { key: 'rentalIncome',               label: 'Huurinkomsten (Box 1)',           hint: 'Bijv. van kamer verhuur eigen woning' },
  { key: 'otherBox1Income',            label: 'Overig Box 1 inkomen',            hint: 'AOW, pensioen, uitkering, etc.' },
  { key: 'mortgageInterestDeduction',  label: 'Hypotheekrente aftrek',           hint: 'Betaalde rente op eigen woning hypotheek' },
...
export default function IncomeSection({ data, onChange }: Props) {
  ...
  const toggle = (key: keyof IncomeData) => {
    setOpen(prev => {
      ...
      if (next.has(key)) {
        next.delete(key);
        onChange({ ...data, [key]: 0 }); // clear value when closing
      } else {
        next.add(key);
      }
      return next;
  ...
  return (
    <SectionCard title="Inkomen — Box 1" icon={<Briefcase size={20} />} accent="border-blue-400">
      <div className="space-y-4">
        ...
        {/* Optional fields as toggleable rows */}
        <div className="space-y-2 pt-1">
          <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">Aanvullende inkomsten &amp; aftrekposten</p>
              ...
              >
                ...
                {data[f.key] > 0 && (
                  <span className="text-xs font-semibold text-blue-600">
                    {new Intl.NumberFormat('nl-NL', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 }).format(data[f.key] as number)}
                  </span>
                )}
              </button>
              {open.has(f.key) && (
